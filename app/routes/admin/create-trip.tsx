...
// Loader que busca os países e já formata o resultado
...
const CreateTrip: React.FC<CreateTripProps> = ({ loaderData }) => {
  ...
  const itemTemplate = (data: any) => (
    <div className="flex items-center">
      <img
        src={data.flagSvg}
        alt={data.text}
        className="w-6 h-4 mr-2.5 ml-2.5"
      />
      <span>{data.text}</span>
    </div>
  );

  // Template para o item selecionado aparecer com imagem + texto
  const valueTemplate = (data: any) => {
    if (!data) {
      return <span className="text-muted">Select a country</span>;
    }
